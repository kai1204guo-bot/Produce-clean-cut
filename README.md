...
封面。多部剧可以加入持久化队列，当前剧结束后自动切换到下一部，适合夜间无人值守。
点击“扫描总文件夹”只需选择一次剧集总目录，程序会递归查找其中的 `成片`、`videos`
或 `video` 文件夹，先显示剧名和集数预览，再批量加入队列；清水版、SRT 和临时目录会
自动排除，重复剧集不会再次加入。
程序用独立的 Chrome 登录目录保存 LibTV 会话，不保存账号或密码。点击“登录 LibTV”
会同时完成网页登录和官方 CLI 授权；安装包不绑定开发者账号，换电脑或换 LibTV 账号后
会自动校验旧画布归属，并在当前账号根目录创建该剧的专用画布。
需要换成另一个用户时点击“切换账号”，在打开的软件专用 LibTV 网页中手动退出并登录
新账号；程序检测到账号变化后自动更新 CLI 授权。任务运行期间禁止切换，且不会删除
本地成品或云端画布。

云端提交并发会按当前会员名称自动调整，并预留 1 个官方名额：标准版 7 个、进阶版
11 个、高级版 19 个；豪华版和至尊版不设软件并发上限。上传仍保持每批最多 15 个文件。

超过 60 秒的单集会在上传前自动均匀切成不超过 58 秒的片段，并在相邻片段保留
0.5 秒上下文。所有片段完成后，程序按原始逐帧时间戳裁掉重叠部分、合并音视频，
只在整集开头恢复一次封面；不会把每段开头的原字幕重新带回成片。SRT 始终从完整
原片生成，不受分段影响。分段状态保存在“清水版”目录内，异常中断后只继续缺失片段。

首次启动会自动检查 FFmpeg/FFprobe、LibTV 官方 CLI、Google Chrome 和随包运行库。
缺少可自动安装的组件时，点击“环境检测 / 一键安装”即可完成下载和安装；NVIDIA
CUDA SRT 加速库属于可选项，未安装时会自动使用 CPU。LibTV 登录仍需用户本人在
浏览器中完成一次授权。
...
构建面向普通用户的 Windows 安装版（需要开发机安装 Inno Setup 6）：

```powershell
.\build_installer.ps1
```

生成文件按版本命名，例如 `installer-dist\清水版批量制作-安装程序-v0.4.1.exe`。安装版会创建开始菜单和
桌面快捷方式，并将程序安装到当前用户目录，不要求管理员权限。外部组件不会在安装
向导中静默捆绑；软件首次打开后会明确列出缺失项，由用户点击后逐项安装，便于看清
安装状态和失败原因。
